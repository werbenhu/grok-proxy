...
func TestConvertResponsesStreamRejectsMalformedAndIncompleteEvents(t *testing.T) {
	tests := []struct {
		name   string
		stream string
	}{
		{name: "malformed json", stream: "event: response.output_text.delta\ndata: {not-json}\n\n"},
		{name: "missing terminal event", stream: "event: response.output_text.delta\ndata: {\"type\":\"response.output_text.delta\",\"delta\":\"partial\"}\n\n"},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			_, err := io.ReadAll(ConvertResponseStream(io.NopCloser(strings.NewReader(test.stream)), OperationChat))
			if err == nil {
				t.Fatal("expected stream conversion error")
			}
		})
	}
}
