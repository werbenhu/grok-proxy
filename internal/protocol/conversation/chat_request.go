...
func convertChatMessages(messages []chatMessage) ([]any, error) {
	...
	for _, message := range messages {
		...
		default:
			return nil, fmt.Errorf("unsupported messages.role=%q", message.Role)
		}
	}
	if len(input) == 0 {
		return nil, errors.New("messages contain no sendable content")
	}
	return input, nil
}
...
func convertChatContent(raw json.RawMessage) (any, error) {
	...
	if err := json.Unmarshal(raw, &parts); err != nil {
		return nil, errors.New("content must be a string or content array")
	}
	...
	for _, part := range parts {
		...
		case "text", "input_text", "output_text":
			...
			if json.Unmarshal(part["text"], &value) != nil {
				return nil, errors.New("invalid text content")
			}
		...
		default:
			return nil, fmt.Errorf("unsupported content.type=%q", typeName)
		}
	}
	return result, nil
}
...
func parseImageURL(part map[string]json.RawMessage) (string, error) {
	...
	return "", errors.New("image_url missing valid url")
}
...
func convertAssistantToolCalls(raw json.RawMessage) ([]any, error) {
	...
	if err := json.Unmarshal(raw, &calls); err != nil {
		return nil, errors.New("invalid assistant.tool_calls format")
	}
	...
	for _, call := range calls {
		if strings.TrimSpace(call.ID) == "" || strings.TrimSpace(call.Function.Name) == "" || !json.Valid([]byte(call.Function.Arguments)) {
			return nil, errors.New("assistant.tool_calls missing valid id, name, or arguments")
		}
...
func convertChatTools(raw json.RawMessage) ([]any, error) {
	...
	for _, tool := range tools {
		...
		if json.Unmarshal(tool["function"], &function) != nil {
			return nil, errors.New("invalid function tool format")
		}
...
func convertChatToolChoice(raw json.RawMessage) (json.RawMessage, error) {
	...
	if json.Unmarshal(value["function"], &function) != nil || strings.TrimSpace(function.Name) == "" {
		return nil, errors.New("invalid tool_choice.function.name")
	}
