...
- `POST /v1/responses`: Native OpenAI Responses JSON / SSE passthrough that preserves image and tool-output structures.
- `POST /v1/responses/compact`: Native non-streaming Responses context-compaction passthrough.
...
For Codex and other native Responses clients, use `http://127.0.0.1:8181/v1` as the Base URL to avoid converting back and forth between Responses and Chat Completions.
